-- ================================================
-- FINAL WORKING VERSION - Stats System Fix
-- ================================================
...
CREATE POLICY "Service can update stats" ON public.site_stats FOR UPDATE USING (false);

-- 2. TRIGGER: Auto-update users_count
CREATE OR REPLACE FUNCTION update_users_count()
RETURNS TRIGGER AS $$
BEGIN
  IF TG_OP = 'INSERT' THEN
    UPDATE public.site_stats SET users_count = users_count + 1, updated_at = NOW() WHERE id = 1;
  ELSIF TG_OP = 'DELETE' THEN
    UPDATE public.site_stats SET users_count = GREATEST(0, users_count - 1), updated_at = NOW() WHERE id = 1;
  END IF;
  RETURN NEW;
END;
...
CREATE TRIGGER trigger_update_users_count AFTER INSERT OR DELETE ON profiles FOR EACH ROW EXECUTE FUNCTION update_users_count();

-- 3. ONLINE COUNT FUNCTIONS
CREATE OR REPLACE FUNCTION get_online_count()
RETURNS INTEGER AS $$
BEGIN
  RETURN (SELECT COUNT(*)::INTEGER FROM user_presence WHERE last_seen >= NOW() - INTERVAL '2 minutes');
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

GRANT EXECUTE ON FUNCTION get_online_count() TO anon, authenticated;

-- 4. MAIN STATS FUNCTION
...
-- 5. FIX USER_PRESENCE RLS (security fix)
...
CREATE POLICY "Users can upsert own presence" ON user_presence FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own presence" ON user_presence;
CREATE POLICY "Users can update own presence" ON user_presence FOR UPDATE USING (auth.uid() = user_id);

-- 6. AUTO-CREATE PROFILES TRIGGER (only id + timestamps)
CREATE OR REPLACE FUNCTION handle_new_user()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.profiles (id, created_at) VALUES (NEW.id, NOW()) ON CONFLICT (id) DO NOTHING;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- 7. BACKFILL existing users (only id + timestamps)
INSERT INTO public.profiles (id, created_at)
SELECT au.id, au.created_at FROM auth.users au
LEFT JOIN public.profiles p ON au.id = p.id
WHERE p.id IS NULL
ON CONFLICT (id) DO NOTHING;

-- 8. SET INITIAL COUNT
UPDATE public.site_stats SET users_count = (SELECT COUNT(*) FROM public.profiles WHERE NOT COALESCE(is_deleted, false)) WHERE id = 1;

-- 9. INDEXES
CREATE INDEX IF NOT EXISTS idx_user_presence_last_seen ON user_presence(last_seen DESC);
CREATE INDEX IF NOT EXISTS idx_profiles_not_deleted ON profiles(id) WHERE NOT COALESCE(is_deleted, false);
